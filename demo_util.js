...
function drawShoulderline(keypoints, minConfidence, ctx, scale = 1) {
    const M = ((keypoints[5].position.y)+(keypoints[6].position.y))/2*1.2;

    if (keypoints[5].score < minConfidence && keypoints[6].score < minConfidence) {
      return;
    }
    drawSegment([M, 0], [M, 530], 'red', scale, ctx);
}

function drawBackline(keypoints, minConfidence, ctx, scale = 1) {
    const A = ((keypoints[5].position.y)+(keypoints[6].position.y))/2;
    const B = ((keypoints[11].position.y)+(keypoints[12].position.y))/2;
    const M = (A+B)*0.65;

    if (keypoints[5].score < minConfidence && keypoints[6].score < minConfidence && keypoints[11].score < minConfidence && keypoints[12].score < minConfidence) {
      return;
    }
    drawSegment([M, 0], [M, 530], 'red', scale, ctx);
}
